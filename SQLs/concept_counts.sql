...
CREATE TEMP TABLE tmp_counts_person
AS
...
CREATE TEMP TABLE tmp_concepts
AS
WITH concepts  AS (
  ...
  from (
    ...
    from tmp_counts c
    join cdm54.concept_ancestor ca on cast(ca.descendant_concept_id as varchar(50)) = c.concept_id
  ) c
  left join cdm54.concept_ancestor ca on c.concept_id = cast(ca.ancestor_concept_id as varchar(50))
)
 SELECT
ancestor_id,
  descendant_id
